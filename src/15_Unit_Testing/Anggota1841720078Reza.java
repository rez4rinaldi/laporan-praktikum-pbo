...
public class Anggota1841720078Reza {
    ...
    public ArrayList<Anggota1841720078Reza> getByNamaAndAlamatAndTeleponReza(String nama, String alamat, String telepon) {
        ArrayList<Anggota1841720078Reza> ListAnggota = new ArrayList();
        ResultSet rs;
        if ((nama.trim().length() > 0) && (alamat.trim().length() > 0) && (telepon.trim().length() > 0)) {
            rs = DBHelper1841720078Reza.selectQueryReza("SELECT * FROM anggota WHERE nama = '" + nama + "'  AND alamat = '" + alamat + "'  AND telepon = '" + telepon + "'");

        } else if ((nama.trim().length() > 0) && (alamat.trim().length() > 0) && (telepon.trim().length() == 0)) {
            rs = DBHelper1841720078Reza.selectQueryReza("SELECT * FROM anggota WHERE nama = '" + nama + "'  AND alamat = '" + alamat + "'");
        } else if ((nama.trim().length() > 0) && (alamat.trim().length() == 0)) {
            rs = DBHelper1841720078Reza.selectQueryReza("SELECT * FROM anggota WHERE nama = '" + nama + "'");
        ...
        try {
            while (rs.next()) {
                Anggota1841720078Reza ang = new Anggota1841720078Reza();
                ang.setIdanggotaReza(rs.getInt("idanggota"));
                ang.setNamaReza(rs.getString("nama"));
                ang.setAlamatReza(rs.getString("alamat"));
                ang.setTeleponReza(rs.getString("telepon"));
                ListAnggota.add(ang);
            }
        } catch (Exception e) {
            e.printStackTrace();
